...
public class StartArmoury : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (CheckTargetTag(hit.collider.gameObject,"Armoury1"))
                {
                    ui.SetActive(true);
                    stringClassName="Armoury1";
                    ValueClass=2;

                }
                if (CheckTargetTag(hit.collider.gameObject,"Armoury2"))
                {
                    ui.SetActive(true);

                    stringClassName="Armoury2";
                    ValueClass=3;

                }
                if (CheckTargetTag(hit.collider.gameObject,"Armoury3"))
                {
                    ui.SetActive(true);

                    stringClassName="Armoury3";
                    ValueClass=4;
    ...
    {
        sauvegardeFonction3.LoadDataFromJson(ValueClass);
        arrayArmoury.DestroyHouse(ValueClass);
        
        sliderHeight.maxValue = sauvegardeFonction3.sliderheight;
        sauvegardeFonction3.height = Mathf.FloorToInt(sliderHeight.value);
       
        RessourceValue.text = "Iron : X"+sliderHeight.value.ToString();
        textHeight.text = $"Height: {sauvegardeFonction3.height - 1f:F2}";

        

        sauvegardeFonction3.retrievedVector3.y = -1f;
        arrayArmoury.BuildHouse(sauvegardeFonction3.length, sauvegardeFonction3.height, sauvegardeFonction3.width, sauvegardeFonction3.retrievedVector3,stringClassName);
    ...
    public void OnClickHeight()
    {
        if (boutonLength != null && boutonLength.IsInteractable() && inventory != null && inventory.puissanceChiffrePairs != null)
        {
            ...
            if(inventory.coinsCountTrue >= aRendreHMoney)
            {
                ...
                inventory.Iron += sliderHeight.value;
