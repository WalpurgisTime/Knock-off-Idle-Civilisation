...
public class StartButchery : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (CheckTargetTag(hit.collider.gameObject,"Butchery1"))
                {
                    ui.SetActive(true);
                    stringClassName="Butchery1";
                    ValueClass=5;

                }
                if (CheckTargetTag(hit.collider.gameObject,"Butchery2"))
                {
                    ui.SetActive(true);

                    stringClassName="Butchery2";
                    ValueClass=6;

                }
                if (CheckTargetTag(hit.collider.gameObject,"Butchery3"))
                {
                    ui.SetActive(true);

                    stringClassName="Butchery3";
                    ValueClass=7;
    ...
    {
        sauvegardeFonction3.LoadDataFromJson(ValueClass);
        arrayButchery.DestroyHouse(ValueClass);
        
        sliderHeight.maxValue = sauvegardeFonction3.sliderheight;
        sauvegardeFonction3.height = Mathf.FloorToInt(sliderHeight.value);
        
        RessourceValue.text = "Chicken : X"+sliderHeight.value.ToString();
        textHeight.text = $"Height: {sauvegardeFonction3.height - 1f:F2}";

        

        sauvegardeFonction3.retrievedVector3.y = -1f;
        arrayButchery.BuildHouse(sauvegardeFonction3.length, sauvegardeFonction3.height, sauvegardeFonction3.width, sauvegardeFonction3.retrievedVector3,stringClassName);
    ...
    public void OnClickHeight()
    {
        if (boutonLength != null && boutonLength.IsInteractable() && inventory != null && inventory.puissanceChiffrePairs != null)
        {
            ...
            if(inventory.coinsCountTrue >= aRendreHMoney)
            {
                ...
                inventory.chicken += sliderHeight.value;
