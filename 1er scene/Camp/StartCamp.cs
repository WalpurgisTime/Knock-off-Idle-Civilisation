...
public class StartCamp : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (CheckTargetTag(hit.collider.gameObject,"Camp1"))
                {
                    ui.SetActive(true);
                    stringClassName="Camp1";
                    ValueClass=10;

                }
                if (CheckTargetTag(hit.collider.gameObject,"Camp2"))
                {
                    ui.SetActive(true);

                    stringClassName="Camp2";
                    ValueClass=11;

                }
                if (CheckTargetTag(hit.collider.gameObject,"Camp3"))
                {
                    ui.SetActive(true);

                    stringClassName="Camp3";
                    ValueClass=12;
    ...
    {
        sauvegardeFonction3.LoadDataFromJson(ValueClass);
        arrayCamp.DestroyHouse(ValueClass);
        
        sliderHeight.maxValue = sauvegardeFonction3.sliderheight;
        sauvegardeFonction3.height = Mathf.FloorToInt(sliderHeight.value);
        
        RessourceValue.text = "Damage : X"+sliderHeight.value.ToString();
        textHeight.text = $"Height: {sauvegardeFonction3.height - 1f:F2}";

        

        sauvegardeFonction3.retrievedVector3.y = -1f;
        arrayCamp.BuildHouse(sauvegardeFonction3.length, sauvegardeFonction3.height, sauvegardeFonction3.width, sauvegardeFonction3.retrievedVector3,stringClassName);
    ...
    public void OnClickHeight()
    {
        if (boutonLength != null && boutonLength.IsInteractable() && inventory != null && inventory.puissanceChiffrePairs != null)
        {
            ...
            if(inventory.coinsCountTrue >= aRendreHMoney)
            {
                ...
                inventory.damage += sliderHeight.value*0.1f;
